...
from task.tests.settings import TEST_ADMIN_EMAIL, TEST_ADMIN_FULL_NAME, TEST_AGE, TEST_GENDER, TEST_PASSWORD
...
class TaskViewTest(APITestCase):
    def setUp(self):
        ...
        self.retrive_report_path = reverse("task:retrive_report", kwargs={"task_id": self.response.json()["task_id"]})
    ...
    def test_retrive_report(self):
        ...
        while response["Content-Type"] != "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet":
            response = self.client.get(path=self.retrive_report_path)
